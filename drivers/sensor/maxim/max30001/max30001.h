...
// struct max30001_encoded_fifo_payload {
// 	union {
// 		uint8_t buf[20];
// 		struct {
// 			uint8_t header;
...
struct max30001_encoded_data {
	struct max30001_encoded_header header;
	struct max30001_encoded_payload payload;
};

struct max30001_triggers {
		...
		K_KERNEL_STACK_MEMBER(thread_stack, CONFIG_MAX30001_THREAD_STACK_SIZE);
...
struct max30001_data {
	...
	struct max30001_triggers triggers;
#elif defined(CONFIG_MAX30001_STREAM)
	struct max30001_stream stream;
...
static inline void max30001_ecg_voltage(uint8_t gain,
					int32_t adc_counts,
					int32_t *out_mv)
{
	// VECG (mV) = ADC x VREF / (2^17 x ECG_GAIN)
	*out_mv = adc_counts * 1000L / (2^17 * 20*2^gain);
}

static inline void max30001_accel_ms(uint8_t fs,
				     int32_t in,
				     bool high_res,
				     int32_t *out_ms,
				     int32_t *out_ums)
...
static inline void max30001_gyro_rads(uint8_t fs,
				      int32_t in,
				      bool high_res,
				      int32_t *out_rads,
				      int32_t *out_urads)
{
	...
	case MAX30001_DT_GYRO_FS_31_25:
		...
		break;
	case MAX30001_DT_GYRO_FS_15_625:
...
static inline void max30001_temp_c(int32_t in, int32_t *out_c, uint32_t *out_uc)
{
