...
LOG_MODULE_REGISTER(MAX30001_STREAM, CONFIG_SENSOR_LOG_LEVEL);
...
static void max30001_handle_event_actions(struct rtio *ctx,
					  const struct rtio_sqe *sqe,
					  void *arg)
{
	const struct device *dev = (const struct device *)arg;
	...
	struct max30001_encoded_data *buf;
	uint32_t buf_len;
	uint32_t buf_len_required = sizeof(struct max30001_encoded_header);
	...
	if (should_read_fifo(read_cfg, int_status)) {
		buf_len_required += (data->stream.data.fifo_count *
				     sizeof(struct max30001_encoded_fifo_payload));
	} else if (should_read_data(read_cfg, int_status)) {
		buf_len_required += sizeof(struct max30001_encoded_payload);
	}
	...
	if (should_read_fifo(read_cfg, int_status)) {
		 ...
		 */
		buf->header.accel_fs = MAX30001_DT_ACCEL_FS_32;
		buf->header.gyro_fs = MAX30001_DT_GYRO_FS_4000;
...
static void max30001_event_handler(const struct device *dev)
{
	struct max30001_data *data = dev->data;
	uint8_t val = 0;
	uint64_t cycles;
...
static void max30001_gpio_callback(const struct device *gpio_dev,
				   struct gpio_callback *cb,
				   uint32_t pins)
{
	struct max30001_stream *stream = CONTAINER_OF(cb,
						      struct max30001_stream,
						      cb);
	const struct device *dev = stream->dev;

	max30001_event_handler(dev);
}

static inline bool settings_changed(const struct max30001_stream *a,
				    const struct max30001_stream *b)
{
			    ...
			    struct rtio_iodev_sqe *iodev_sqe)
{
	 ...
	 * in such case.
	 */
	struct max30001_stream stream = {0};
	...
	if (settings_changed(&data->stream, &stream)) {
		...
		err = max30001_bus_write(dev, REG_INT1_CONFIG0, &val, 1);
		if (err) {
		...
		err = max30001_bus_read(dev, REG_INT1_STATUS0, &val, 1);
		if (err) {
		...
		err = max30001_bus_write(dev, REG_FIFO_CONFIG3, &val, 1);
		if (err) {
		...
		err = max30001_bus_write(dev, REG_FIFO_CONFIG0, &val, 1);
		if (err) {
		...
		if (data->stream.settings.enabled.fifo_ths ||
			...
			val = REG_FIFO_CONFIG2_FIFO_WM_GT_THS(true) |
			      REG_FIFO_CONFIG2_FIFO_FLUSH(true);
			err = max30001_bus_write(dev, REG_FIFO_CONFIG2, &val, 1);
			if (err) {
			...
			err = max30001_bus_write(dev, REG_FIFO_CONFIG1_0, (uint8_t *)&fifo_ths, 2);
			if (err) {
...
int max30001_stream_init(const struct device *dev)
{
	...
	err = max30001_bus_write(dev, REG_INT1_CONFIG2, &val, 1);
	if (err) {
