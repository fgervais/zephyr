...
LOG_MODULE_REGISTER(MAX30001_TRIGGER, CONFIG_SENSOR_LOG_LEVEL);

static void max30001_gpio_callback(const struct device *dev,
				   struct gpio_callback *cb,
				   uint32_t pins)
{
	struct max30001_data *data = CONTAINER_OF(cb,
...
static void max30001_thread(void *p1, void *p2, void *p3)
{
	ARG_UNUSED(p2);
	ARG_UNUSED(p3);

	struct max30001_data *data = p1;

	while (true) {
		k_sem_take(&data->triggers.sem, K_FOREVER);

		max30001_thread_cb(data->triggers.dev);
	}
}

#elif defined(CONFIG_MAX30001_TRIGGER_GLOBAL_THREAD)

static void max30001_work_handler(struct k_work *work)
{
	struct max30001_data *data = CONTAINER_OF(work,
						  struct max30001_data,
						  triggers.work);
...
static int max30001_enable_drdy(const struct device *dev, bool enable)
{
	uint8_t val;
	int err;

	err = max30001_bus_read(dev, REG_INT1_CONFIG0, &val, 1);
	if (err) {
		return err;
	}
	...
	return max30001_bus_write(dev, REG_INT1_CONFIG0, &val, 1);
}
			 ...
			 sensor_trigger_handler_t handler)
{
	...
	case SENSOR_TRIG_DATA_READY:
		...
		if (handler) {
			...
			err = max30001_enable_drdy(dev, true);
		} else {
			/* Disable data ready interrupt */
			err = max30001_enable_drdy(dev, false);
		}
		break;
	default:
		err = -ENOTSUP;
...
int max30001_trigger_init(const struct device *dev)
{
	...
	(void)k_thread_create(&data->triggers.thread,
			      ...
			      NULL,
			      NULL,
			      K_PRIO_COOP(CONFIG_MAX30001_THREAD_PRIORITY),
			      0,
			      K_NO_WAIT);

#elif defined(CONFIG_MAX30001_TRIGGER_GLOBAL_THREAD)
	k_work_init(&data->triggers.work, max30001_work_handler);
#endif
